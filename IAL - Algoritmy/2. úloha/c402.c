...
**/
	tStackP TmpStack;
	SInitP(&TmpStack);
	Leftmost_Preorder(RootPtr, &TmpStack);

	while (!SEmptyP(&TmpStack)){
		RootPtr = STopPopP(&TmpStack);
		Leftmost_Preorder(RootPtr->RPtr, &TmpStack);
...
**/
	tStackP TmpStack;
	SInitP(&TmpStack);
	Leftmost_Inorder(RootPtr, &TmpStack);

	while (!SEmptyP(&TmpStack)){
		RootPtr = STopPopP(&TmpStack);
		BTWorkOut(RootPtr);
		Leftmost_Inorder(RootPtr->RPtr, &TmpStack);
...
**/
	...
	Leftmost_Postorder(RootPtr,&StackA, &StackB);

	while (!SEmptyP(&StackA)){
		RootPtr = STopPopP(&StackA);
		adasafsa = STopPopB(&StackB); 
		if(adasafsa == true){
			if (RootPtr->RPtr == NULL){
				
				BTWorkOut(RootPtr);
			}else{
				SPushP(&StackA, RootPtr);
				SPushB(&StackB, false);
				Leftmost_Postorder(RootPtr->RPtr, &StackA, &StackB);
			}
...
**/
	...
	do{
		if ((*RootPtr) == NULL){
			...
			if ((*RootPtr)->RPtr != NULL){
				SPushP(&TmpStack, (*RootPtr)->RPtr);
			}
			tBTNodePtr tmpptr = (*RootPtr);
			(*RootPtr) = (*RootPtr)->LPtr;
			free(tmpptr);
		}
	} while(((*RootPtr) != NULL) || ( !SEmptyP(&TmpStack) ) );
