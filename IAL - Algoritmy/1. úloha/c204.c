...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
    ...
    while ( stackEmpty(s) == 0){ // vyprázdnenie zásobníku
        stackTop(s,&temp);
        stackPop(s);
		if(temp == '('){ //ak je na vrchole zásobníku zátvorka už neberie viac znakov
            break;
        }

        postExpr[*postLen] = temp; //pridanie na koniec reťazca
		(*postLen)++; //zväčšená dlžka reťazca
    }
}
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
    ...
    if (stackEmpty(s) != 0){ // overenie či je zásobník prázdny... vtedy vloží operátor na vrchol zásobníku a nič nerieši
     	stackPush(s, c);
		return;
    }
    ...
    if (temp == '+' || temp == '-'){ // ak je na vrchole zásobníku operátor s menšou hodnotou pridá len na koniec zásobníku
		if (c == '*' || c == '/'){
			stackPush(s, c); 
			return;
		} 
	}
	if (temp == '('){ // ak je na vrchole ( 
      	stackPush(s, c); 
		return;
    }
    
    postExpr[*postLen] = temp; // pridanie na koniec reťazca
	(*postLen)++; // inkrementácia dlžky reťazca
    stackPop(s); 
    doOperation(s, c, postExpr, postLen); //volanie funkcie pokiaľ sa c nepodarí pridať
} 
...
char* infix2postfix (const char* infExpr) {
  	char *posttmp = (char *) malloc(MAX_LEN * sizeof(char)); // alokácia pre pomocnú premennú
	if (!posttmp){
    	return NULL;
	}
 	tStack *stacktmp = (tStack *) malloc(sizeof(tStack)); // alokácia stacku
	if (!stacktmp){
		free(posttmp);
		return NULL;
	}
	...
	unsigned int x = 0; 
	char c;
	for ( c = *infExpr; c != '\0'; c = *(++infExpr)){ // pokiaľ nieje koniec infixu berie znak po znaku a ukladá do premennej c
		if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
			posttmp[x] = c;
			x++;
		}else if (c == '(') {
			stackPush(stacktmp, c); 
		}else if (c == '+' || c == '-' || c == '*' || c == '/') {
			doOperation(stacktmp, c, posttmp, &x); 
		}else if (c == ')') {
			untilLeftPar(stacktmp, posttmp, &x); 
		}else if (c == '=') {
			while (stackEmpty(stacktmp) == 0){ // prepis zásobníku na výstup pokiaľ nebude zásobník prázdny, lebo už bolo zadané =
				stackTop(stacktmp, &(posttmp[x]));
				x++;
				stackPop(stacktmp); 
			}
			posttmp[x] = '='; // ak bolo rovná sa tak za ním dá \0 aby nemohlo nič nasledovať
			x++; 
			posttmp[x] = '\0'; 
			x++;
		}
	...
	free(stacktmp); // uvolnenie pamete alokovanej
